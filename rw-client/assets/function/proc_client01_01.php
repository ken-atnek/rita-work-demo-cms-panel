...
if (!is_array($prevSearchConditions)) {
	$prevSearchConditions = [
		'facility_id' => $facId,
		'searchMode' => ($searchMode !== '' ? $searchMode : 'applied'),
...
#-------------#
$prevSearchMode = isset($prevSearchConditions['searchMode']) ? (string)$prevSearchConditions['searchMode'] : 'applied';
...
$searchMode = isset($searchConditions['searchMode']) && (string)$searchConditions['searchMode'] !== '' ? (string)$searchConditions['searchMode'] : 'applied';
...
foreach ($applicationClientSetting as $statusKey => $status) {
	$applicationCounts[$statusKey] = (int)getApplicationCount($statusKey, (int)$facId);
}
#キーが無い場合も想定して0で補完
					...
					if (isset($applicationStatusClient) && is_array($applicationStatusClient) && count($applicationStatusClient) > 0) {
						foreach ($applicationStatusClient as $appStatusKey => $appStatus) {
                          ...
                          <li {$zIndexStyleStatus}>
                            <input type="radio" name="application_status{$jobKey}" value="{$appStatusKey}" id="application_status{$jobKey}-{$appStatusKey}" {$checked} onchange="changeApplicationStatus({$appliedJobsFacIdInt}, {$lineUserIdJsAttr}, {$sendStatusChangeNameJsAttr}, {$jobIdInt}, this.value, {$searchModeJsAttr}, {$sortModeJsAttr});">
