				...
				if ($accountData) {
					try {
						...
						if ($result == false) {
							#エラーログ出力
							$data = [
								'pageName' => 'master_reset_password',
								'reason' => 'トランザクション開始失敗',
							];
							makeLog($data);
							$errorFlg = 1;
							...
							if ($dbSuccessFlg == 1) {
								...
								DB_Transaction(2);
								#パスワード再設定用URL生成
								$resetUrl = DOMAIN_NAME . "/demo-cms-panel/rw-client/resetPassword.php?selector=" . $selector . "&validator=" . bin2hex($token);
								#メール送信処理
