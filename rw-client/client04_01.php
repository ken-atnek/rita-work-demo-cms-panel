...
if ($facId !== null) {
  ...
  #事業所ID無し：処理終了
  header("Location: ./client04_01.php");
  exit;
}
...
if (isset($_SESSION[$searchConditionsSessionKey]) === false || !is_array($_SESSION[$searchConditionsSessionKey])) {
  ...
  $_SESSION[$searchConditionsSessionKey] = array(
    'facility_id' => $facId,
    #事業所側の「全て」表示時は登録中/連絡待ち等を除外
    'excludeStatuses' => ['registered', 'unresponsive', 'friend_only'],
    'referName' => '',
    'referJobType' => '',
    'startDay' => '',
    'endDay' => '',
...
foreach ($requiredKeys as $requiredKey) {
  if (!array_key_exists($requiredKey, $searchConditions)) {
    ...
    $searchConditions = array(
      ...
      'excludeStatuses' => (isset($searchConditions['excludeStatuses']) && is_array($searchConditions['excludeStatuses'])) ? (array)$searchConditions['excludeStatuses'] : ['registered', 'unresponsive', 'friend_only'],
...
#inline JS用エスケープ（ヘッダー操作：表示件数/ソート）
...
HTML;
#職種が選択されている場合
$currentReferJobType = $searchConditions['referJobType'] ?? '';
if ($currentReferJobType !== '') {
  foreach ($jobCategories as $jobCategory) {
    if ((string)$currentReferJobType === (string)($jobCategory['id'] ?? '')) {
      ...
      print <<<HTML
                      <input type="hidden" name="referJobType" value="{$jobCategoryIdEsc}" data-selectbox-hidden>
                      <span class="selectbox__value" data-selectbox-value>{$jobCategoryNameEsc}</span>
...
if (isset($jobCategories) && is_array($jobCategories) && count($jobCategories) > 0) {
  foreach ($jobCategories as $jobCategory) {
    #checked判定
    $checked = ((string)$currentReferJobType === (string)($jobCategory['id'] ?? '')) ? 'checked' : '';
    $jobCategoryIdEsc = htmlspecialchars((string)($jobCategory['id'] ?? ''), ENT_QUOTES, 'UTF-8');
    $jobCategoryNameEsc = htmlspecialchars((string)($jobCategory['name'] ?? ''), ENT_QUOTES, 'UTF-8');
    print <<<HTML
                        <li>
...
$currentSearchModeUi = '';
if ($currentSearchMode !== '' && isset($applicationStatusClient[(string)$currentSearchMode])) {
...
if ($currentSearchModeUi !== '') {
  ...
  $appStatusEsc = htmlspecialchars((string)$applicationStatusClient[$currentSearchModeUi], ENT_QUOTES, 'UTF-8');
  print <<<HTML
...
HTML;
  ...
  print <<<HTML
                        <!-- NOTE  インラインでz-indexを付与 -->
                        <li style="z-index: 2">
                          <input type="radio" name="searchMode" value="2" id="application_status-none">
                          <label for="application_status-none" class="status-applied">応募状況ステータスが未設定です</label>
                        </li>
...
HTML;
            ...
            print <<<HTML
                        <input type="hidden" name="application_status{$jobKey}" value="1" data-selectbox-hidden>
                        <span class="selectbox__value" data-selectbox-value>選択してください</span>
                        <i></i>
