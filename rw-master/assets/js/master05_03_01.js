 ...
 *
 */
const requestURL = './assets/function/proc_master05_03_01.php';
 ...
 */
const __notificationStatusPrevByNotificationId = Object.create(null);
let __notificationStatusPending = null;
function __getStatusSelectBoxParts(notificationId) {
  const name = `list_status${notificationId}`;
...
function __applyStatusToUi(notificationId, statusValue) {
  const { box, head, valueEl, hiddenEl, radios } = __getStatusSelectBoxParts(notificationId);
...
function __setModalCloseToCancel() {
  ...
  btn.setAttribute('onclick', 'cancelNotificationStatusChange();');
}
...
function cancelNotificationStatusChange() {
  try {
    if (__notificationStatusPending && __notificationStatusPending.notificationId) {
      const { notificationId, prevValue } = __notificationStatusPending;
      if (prevValue) __applyStatusToUi(notificationId, prevValue);
    }
  } finally {
    __notificationStatusPending = null;
    __restoreModalCloseDefault();
    closeModal();
  }
}
window.cancelNotificationStatusChange = cancelNotificationStatusChange;
...
document.addEventListener(
  ...
  (e) => {
    ...
    if (!m) return;
    const notificationId = parseInt(m[1], 10);
    if (!Number.isFinite(notificationId)) return;
    const { hiddenEl } = __getStatusSelectBoxParts(notificationId);
    const prev = hiddenEl ? String(hiddenEl.value || '').trim() : '';
    if (prev) __notificationStatusPrevByNotificationId[String(notificationId)] = prev;
  },
  true
);
/**
...
async function requestCorporations({ action, sortMode, pageNumber }) {
  ...
  if (data && data.status === 'error') {
    ...
    location.href = './master05_03_01.php';
 ...
 *
 */
function checkNotificationStatus(notificationId, notificationCode, status) {
  let blockModal = document.getElementById('modalBlock');
  const prevValue =
    __notificationStatusPrevByNotificationId[String(notificationId)] ||
    __getStatusSelectBoxParts(notificationId).hiddenEl?.value ||
    '';
  //同じ値を選んだだけなら何もしない（modalが出続けるのを防ぐ）
  if (String(prevValue || '').trim() === String(status || '').trim()) {
    return;
  }
  __notificationStatusPending = {
    ...
    case 'draft':
      {
        blockModal.querySelector('.box-details p').innerHTML =
          'お知らせ番号：' + notificationId + 'を「下書き中」に変更します。よろしいですか？';
      }
      break;
    //「公開」へ
    case 'public':
      {
        blockModal.querySelector('.box-details p').innerHTML =
          'お知らせ番号：' + notificationId + 'を「公開中」に変更します。よろしいですか？';
      }
      break;
  }
  //キャンセルボタン生成
  let cancelButton =
    '<button type="button" class="btn-cancel" onclick="cancelNotificationStatusChange();">キャンセル</button>';
  blockModal.querySelector('.box-btn').insertAdjacentHTML('beforeend', cancelButton);
  //登録ボタン生成
  const safeCode = String(notificationCode || '')
    .replace(/\\/g, '\\\\')
    .replace(/'/g, "\\'")
    .replace(/\r\n|\r|\n/g, '\\n');
  let addButton = `<button type="button" class="btn-confirm" onclick="changeNotificationStatus(${notificationId},'${safeCode}','${status}');">はい</button>`;
 ...
 *
 */
async function changeNotificationStatus(notificationId, notificationCode, status) {
  ...
  cFd.append('changeNotificationId', notificationId);
  cFd.append('changeNotificationCode', notificationCode);
  ...
  try {
    ...
    if (list['status'] == 'error') {
      ...
      if (__notificationStatusPending && __notificationStatusPending.notificationId) {
        const prev = String(__notificationStatusPending.prevValue || '').trim();
        if (prev) __applyStatusToUi(__notificationStatusPending.notificationId, prev);
      }
      __notificationStatusPending = null;
      __restoreModalCloseDefault();
      blockModal.classList.add('bg-orange');
      ...
      //一覧へ戻るボタン生成
      let newButton = `<button type="button" class="btn-cancel" onclick="closeModalToPage('master05_03_01.php');">一覧に戻る</button>`;
      blockModal.querySelector('.box-btn').insertAdjacentHTML('beforeend', newButton);
      //「✕」ボタンも変更
      blockModal
        .querySelector('.box-title')
        .querySelector('button')
        .setAttribute('onclick', "closeModalToPage('master05_03_01.php')");
    }
    blockModal.classList.add('is-active');
    document.documentElement.style.overflow = 'hidden';
  } catch (error) {
