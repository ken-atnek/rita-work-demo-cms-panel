 ...
 *
 */
const requestURL = './assets/function/proc_master03_01_01.php';
/**
 * メールアドレスアカウントチェック
 *
 */
...
async function sendInput() {
  ...
  try {
    ...
    if (list['status'] == 'error') {
      ...
      //一覧へ戻るボタン生成
      let newButton =
        '<button type="button" class="btn-cancel" onclick="closeModalToPage(\'master03_01.php\');">一覧に戻る</button>';
      blockModal.querySelector('.box-btn').insertAdjacentHTML('beforeend', newButton);
      //求人カード作成ボタン生成
      let addButton =
        '<button type="button" class="btn-cancel" onclick="closeModalToPage(\'master03_02_01.php?method=new&facId=' +
        list['facId'] +
        '\');" style="width:26rem;">続けて求人カードを登録する</button>';
      blockModal.querySelector('.box-btn').insertAdjacentHTML('beforeend', addButton);
      //「✕」ボタンも変更
      blockModal
        .querySelector('.box-title')
        .querySelector('button')
        .setAttribute('onclick', "closeModalToPage('master03_01.php')");
    }
    blockModal.classList.add('is-active');
    document.documentElement.style.overflow = 'hidden';
    //リクエスト完了後は解除
