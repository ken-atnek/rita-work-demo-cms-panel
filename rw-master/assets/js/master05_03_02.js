/**
 * master05_03_02.js
 * - 事業所へのお知らせ：登録/編集（AJAX）
 * - DB保存 / JSON書き出し / 画像保存は proc_master05_03_02.php
 */
const requestURL = './assets/function/proc_master05_03_02.php';
...
window.rwTipTapUploadImage = async (file) => {
  ...
  fd.append('notificationId', getInputValue('notificationId', '0'));
  fd.append('file', file);
...
function openSuccessModal(message) {
  ...
  if (titleEl) titleEl.textContent = '新規お知らせ情報';
  if (msgEl) msgEl.textContent = message || '登録が完了しました。';
  //ボタン再生成（成功時：一覧へ戻る）
  const btnWrap = blockModal.querySelector('.box-btn');
  if (btnWrap) {
    btnWrap.innerHTML =
      '<button type="button" class="btn-cancel" onclick="closeModalToPage(\'./master05_03_01.php\')">一覧に戻る</button>';
  }
...
function ensureEditorContentFromConfig() {
  ...
  const initCfg = window.RW_MASTER05_03_02 || {};
...
async function checkInput() {
  try {
    ...
    if (!title) {
      openErrorModal('お知らせ登録失敗', 'タイトルを入力してください。');
      return;
    }
    //表示期間（オプション）
    ...
    console.error(error);
    openErrorModal('お知らせ登録失敗', String(error?.message || error));
  }
}
/**
 * 入力内容修正
 ...
 *
 */
function checkDeleteNotification(notificationId, noUpDateKey) {
  //確認メッセージ
  let blockModal = document.getElementById('modalBlock');
  //削除確認メッセージ
  blockModal.querySelector('.box-title p').innerHTML = 'お知らせ削除';
  blockModal.querySelector('.box-details p').innerHTML = `お知らせを削除します。よろしいですか？`;
  //ボタン再生成
  ...
  let newButton =
    ...
    `<button type="button" class="btn-confirm" onclick="deleteNotification('${notificationId}','${noUpDateKey}');">削除する</button>`;
 ...
 *
 */
async function deleteNotification(notificationId, noUpDateKey) {
  //formData生成
  let dFd = new FormData();
  dFd.append('method', 'delete');
  dFd.append('action', 'sendInput');
  dFd.append('notificationId', String(notificationId));
  ...
  try {
    ...
    if (list['status'] == 'error') {
      ...
      blockModal.querySelector('.box-details p').innerHTML =
        'お知らせの削除に失敗しました。<br>お手数ですが最初からやり直してください。';
      //ボタン再生成
      let buttonList = blockModal.querySelector('.box-btn').querySelectorAll('button');
      //ボタンタグを削除
      buttonList.forEach((ElementButton) => {
    ...
    openErrorModal('お知らせ削除失敗', String(error?.message || error));
  } finally {
    window.__rwUploadDraftDiscardDisable = false;
  }
}
...
window.checkDeleteNotification = checkDeleteNotification;
